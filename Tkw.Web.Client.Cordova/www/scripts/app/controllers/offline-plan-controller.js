...
(function (RapApp) {
    ...
    (function (Controllers) {
        var images = [];
        var OfflinePlanController = (function (_super) {
            __extends(OfflinePlanController, _super);
            // initializes the controller
            function OfflinePlanController($scope) {
                var _this = this;
                this.$scope = $scope;
                ...
                this.$scope.growCanvas = function () {
                    ...
                    $("#respondCanvas").attr("style", "margin-right:25px");
                    $("#respondCanvas").attr("style", "margin-bottom:25px");
                ...
                this.$scope.navigateToPlan = function (view) {
                    if (view.Id != $scope.CurrentPlan.Id) {
                        _this.navigateUrl('OfflinePlan', view.Id);
                    }
                };
            ...
            OfflinePlanController.prototype.loadPlan = function (id) {
                var _this = this;
                var self = this;
                this.$scope.IsLoading = true;
                TKWApp.Data.DataManager.Collections["OfflineBuildingPlans"].find(id).then(function (data) {
                    ...
                    //query.and().eq("Id", data.BuildingId);
                    //TKWApp.Data.DataManager.Collections["OfflineBuildings"].search(query).then(function (data) {
                    //    debugger;
                    //    _this.$scope.Building = data;
                    //}, function (success) {
                    //}, function (error) {
            ...
            OfflinePlanController.prototype.toggleEditMode = function () {
                var scope = this;
            ...
            OfflinePlanController.prototype.loadOtherPlans = function () {
                var self = this;
                var query = new TKWApp.Data.Query();
                query.and().eq("BuildingId", self.BuildingId);
                TKWApp.Data.DataManager.Collections["OfflineBuildingPlans"].search(query).then(function (data) {
            ...
            OfflinePlanController.prototype.loadHotspotTypes = function () {
                var self = this;
                self.$scope.HotspotDisplayTypes = [];
                self.$scope.HotspotPoints = [];
                self.$scope.PinHotspots = [];
                TKWApp.Data.DataManager.Collections["OfflineHotspotDisplayTypes"].search(null).then(function (data) {
                ...
                TKWApp.Data.DataManager.Collections["OfflineHotspotActionTypes"].search(null).then(function (data) {
            ...
            OfflinePlanController.prototype.createHotspot = function (displayType, posX, posY, width, height, color, posX1, posY1, posX2, posY2, bId, isPin) {
                ...
                //return;
                self.$scope.CurrentPlan.Hotspots.push(hotspotDTO);
                ...
                self.initFabricHotspot(spot);
                self.$scope.CurrentPlan.HotspotsCount = self.$scope.CurrentPlan.Hotspots.length;
                TKWApp.Data.DataManager.Collections["OfflineBuildingPlans"].update(self.$scope.CurrentPlan).then(function (data) { }, function (success) { }, function (error) { });

                //var plans = self.$scope.Building[0].BuildingPlans;
                //for (var i = 0; i < plans.length; i++) {
                //    if (self.PlanId == plans[i].Id) {
                //        self.$scope.Building[0].BuildingPlans[i].Hotspots.push(spot);
                //        TKWApp.Data.DataManager.Collections["OfflineBuildings"].update(self.$scope.Building[0]).then(function (data) { }, function (success) { }, function (error) { });
                //        TKWApp.Data.DataManager.Collections["OfflineBuildingPlans"].update(self.$scope.CurrentPlan).then(function (data) { }, function (success) { }, function (error) { });
                //        break;
                //    }
                //}
                //TKWApp.Data.DataManager.Collections["OfflineBuildingPlans"].find(id).then(function (data) {
                //    var hps = data.Hotspots;
                //    var idx = -1;
                //    for (var h = 0; h < hps.length; h++) {
                //        if (hps[h].Id == sel.Dto.Id) {
                //            idx = h;
                //            break;
                //        }
                //    }
            ...
            OfflinePlanController.prototype.deleteSelectedSpot = function () {
                var scope = this;
                var self = scope.Controller;
                ...
                if (dto) {
                    ...
                    var idx = scope.CurrentPlanHotspots.length;
                    ...
                    while (idx > 0) {
                        for (var c = 0; c < scope.CurrentPlanHotspots.length; c++) {
                            ...
                            if (scope.CurrentPlanHotspots[c].Dto.BeaconuuId == dto.Id) {
                    ...
                    idx = scope.CurrentPlan.Hotspots.length;
                    while (idx > 0) {
                        for (var c = 0; c < scope.CurrentPlan.Hotspots.length; c++) {
                            ...
                            if (scope.CurrentPlan.Hotspots[c].BeaconuuId == dto.Id) {
                                scope.CurrentPlan.Hotspots.splice(c, 1);
                            }
                        }
                        idx--;
                    ...
                    self.$scope.CurrentPlan.HotspotsCount = self.$scope.CurrentPlan.Hotspots.length;
                    TKWApp.Data.DataManager.Collections["OfflineBuildingPlans"].update(scope.CurrentPlan).then(function (data) { }, function (success) { }, function (error) { });
                    self.canvas.renderAll();
            ...
            OfflinePlanController.prototype.updateSelectedHotspot = function () {
                var scope = this;
                scope.IsSaving = true;
                ...
                var hps = scope.CurrentPlan.Hotspots;
                for (var i = 0; i < hps.length; i++) {
                    if (hps[i].Id == scope.SelectedHotspot.Dto.Id) {
                        
                        if (images.length > 0) {
                            var img = {
                                Id: "hpFile_" + generateUUID(),
                                BucketName: scope.CurrentPlan.PlanFile.BucketName + "/" + scope.CurrentPlan.Id.replace("plan_",""),
                                BucketPath: 'Files',
                                FileDescription: "",
                                FileId: "",
                            ...
                            fileTrn.download_file(images[0], img.BucketPath + "/" + img.BucketName, img.FileName, function (theFile) { }, function (error) { });
                        }
                        scope.SelectedHotspot.Dto.Files.push(img);
                        hps.splice(i, 1);
                        hps.push(scope.SelectedHotspot.Dto);
            ...
            OfflinePlanController.prototype.editSelectedHotspot = function () {
                $("#flName").html("");
                images = [];
                var scope = this;
                var self = scope.Controller;
            ...
            OfflinePlanController.prototype.getPhoto = function () {
                var scope = this;
                var scope = this;
                var self = scope.Controller;
                ...
                if ($("#select-clients option[value='3']").attr("selected")) {
                    mediaType = navigator.camera.MediaType.VIDEO;
                }
                else if ($("#select-clients option[value='4']").attr("selected")) {
                    mediaType = navigator.camera.MediaType.ALLMEDIA;
                }
                else if ($("#select-clients option[value='5']").attr("selected")) {
                    mediaType = navigator.camera.MediaType.ALLMEDIA;
                }
            ...
            OfflinePlanController.prototype.customizeObject = function () {
                var scope = this;
                var self = scope.Controller;
                var obj = self.getSelectedHotspot();
                if (obj) {
            ...
            OfflinePlanController.prototype.updateCustomizeObject = function () {
                var scope = this;
                var self = scope.Controller;
                var obj = self.getSelectedHotspot();
            ...
            OfflinePlanController.prototype.customizePinedObjects = function () {
                var scope = this;
                var self = scope.Controller;
                var obj = self.getSelectedHotspot();
                if (obj) {
            ...
            OfflinePlanController.prototype.updatePinedObjectCustomProperties = function () {
                var scope = this;
                var self = scope.Controller;
                var obj = self.getSelectedHotspot();
                var HotspotCustomPropertiesDto = {
            ...
            OfflinePlanController.prototype.reloadPlan = function () {
                var self = this;
                localStorage.setItem("Reload", "true");
                window.location.href = "plan-offline.html?id=" + self.Controller.PlanId;
            };
            OfflinePlanController.prototype.getPlanImage = function (plan) {
                var fileLink = RapApp.FileUtils.getImageUrl(plan.PlanFile.BucketPath, plan.PlanFile.BucketName, plan.PlanFile.FileName);
                return fileLink;
            };
            OfflinePlanController.prototype.getSpotFileUrl = function (spotFile) {
                if (!spotFile)
                    return "";
                var fileLink = RapApp.FileUtils.getImageUrl(spotFile.BucketPath, spotFile.BucketName, spotFile.FileName);
                return fileLink;
            };
            OfflinePlanController.prototype.getSpotFileType = function (spotFile) {
                if (!spotFile)
                    return "";
                var type = RapApp.FileUtils.getFileType(spotFile.FileName);
                return type;
            };
            OfflinePlanController.prototype.isSpotFileType = function (spotFile, fType) {
                var type = RapApp.FileUtils.getFileType(spotFile.FileName) == fType;
                return type;
            };
            OfflinePlanController.prototype.getHotspotDisplayTypeImage = function (displayType) {
                return RapApp.FileUtils.getHotspotDisplayImage(displayType.FileName);
            };
            OfflinePlanController.prototype.removeHotspotFile = function (item) {
                var scope = this;
                scope.SelectedHotspot.Dto.Files.splice(scope.SelectedHotspot.Dto.Files.indexOf(item), 1);
                TKWApp.Data.DataManager.Collections["OfflineBuildingPlans"].update(scope.CurrentPlan).then(function (data) { }, function (success) { }, function (error) { });
            };
            OfflinePlanController.prototype.hotspotActionTypeAllowsAttachment = function () {
                if ($("#select-clients option[value='1']").attr("selected"))
                    return false;
                else
                    return true;
            };
            OfflinePlanController.prototype.reloadHotspot = function (hotspot) {
            ...
            OfflinePlanController.prototype.getSelectedHotspot = function () {
                var self = this;
                var currentObject = self.canvas.getActiveObject();
                // seach the spot with this object
            ...
            OfflinePlanController.prototype.getLastPinHotspot = function () {
                var self = this;
            ...
            OfflinePlanController.prototype.undoPin = function () {
                var scope = this;
                var self = scope.Controller;
                var obj = self.getSelectedHotspot();
                if (obj) {
                    ...
                    for (var c = 0; c < scope.CurrentPlan.Hotspots.length; c++) {
                        if (scope.CurrentPlan.Hotspots[c].BeaconuuId == obj.Dto.Id) {
                            scope.CurrentPlan.Hotspots[c].Dto.BeaconuuId = null;
                        }
                    }
                    for (var g in self.hashPinedLineTable2) {
            ...
            OfflinePlanController.prototype.setPin = function () {
                var scope = this;
                var self = scope.Controller;
                scope.PinAct = true;
            ...
            OfflinePlanController.prototype.reloadSetPin = function () {
                var scope = this;
                var self = scope.Controller;
                var sel = self.getSelectedHotspot();
                var cx = 1;
            ...
            // CANVAS UTILS
            OfflinePlanController.prototype.initFabric = function (successFunction) {
                var _this = this;
                var self = this;
                // initialize canvas 
                        ...
                        if (self.hashPinedLineTable2[sel.Dto.Id]) {
                            ...
                            for (var i = 0; i < self.$scope.CurrentPlanHotspots.length; i++) {
                                if (self.$scope.CurrentPlanHotspots[i].Dto.Id == dto.Dto.Id) {
                                    ...
                                    //    if (self.PlanId == plans[j].Id) {

                                    //        var hps = plans[j].Hotspots;
                                    //        for (var h = 0; h < hps.length; h++) {
                                    //            if (hps[h].Id == self.$scope.CurrentPlanHotspots[i].Dto.Id) {
                                    //                hps[h].DisplayDetails = self.$scope.CurrentPlanHotspots[i].Dto.DisplayDetails;
                                    //                TKWApp.Data.DataManager.Collections["OfflineBuildings"].update(self.$scope.Building[0]).then(function (data) {
                                    //                }, function (success) {
                                    //                }, function (error) {
                                    //                });
                                    //                break;
                                    //            }
                                    //        }
                                    //        TKWApp.Data.DataManager.Collections["OfflineBuildingPlans"].update(plans[j]).then(function (data) { }, function (success) { }, function (error) { });
                                    //        break;
                                    //    }
                                    //    // save to storage
                                    //    //SAVE localy
            ...
            OfflinePlanController.prototype.hotspotDragging = function (ev, draggable, hotspotDisplayType) {
            ...
            OfflinePlanController.prototype.hotspotDropped = function (ev, droppable) {
                var scope = this;
                // get real position
            ...
            OfflinePlanController.prototype.initFabricHotspot = function (spot) {
                var _this = this;
            ...
            OfflinePlanController.prototype.deleteActiveFabricObject = function () {
                var obj = this.canvas.getActiveObject();
                this.canvas.remove(obj);
            };
            OfflinePlanController.prototype.deleteFabricObject = function (obj) {
                this.canvas.remove(obj);
            };
            //mobile function
            OfflinePlanController.prototype.pressHp = function (item) {
                var $scope = this;
                $scope.$parent.PressedHp = item;
                $scope.$parent.HasPressedHp = true;
            };
            OfflinePlanController.prototype.dropHp = function (event) {
            ...
            return OfflinePlanController;
        })(Controllers.BaseController);
        Controllers.OfflinePlanController = OfflinePlanController;
        ...
        function onPhotoURISuccess(imageURI) {
            ...
            $("#flName").html(imageURI.substr(imageURI.lastIndexOf('/') + 1));
