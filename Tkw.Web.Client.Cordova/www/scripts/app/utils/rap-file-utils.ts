    ...
    var FileUtils = (function () {
        ...
        FileUtils.getImageUrl = function (bucketPath, bucketName, fileName) {
            ...
            if (TKWApp.Configuration.ConfigurationManager.WorkMode === TKWApp.Configuration.WorkMode.ONLINE) {
                ...
                return TKWApp.Configuration.ConfigurationManager.LocalUri + bucketPath + '/' + this.escapeHtml(bucketName) + '/' + this.escapeHtml(fileName);
            }
        };
        FileUtils.getOnlineHotspotDisplayImage = function (path) {
            if (TKWApp.Configuration.ConfigurationManager.WorkMode === TKWApp.Configuration.WorkMode.ONLINE) {
                ...
                return TKWApp.Configuration.ConfigurationManager.LocalUri + "/Content/Images/Hotspots/" + path;
            }
        };
        ...
        FileUtils.getFileType = function (path) {
            ...
            if (/\.(jpg|png|gif|bmp)$/.test(path)) {
                return "image";
            }
            if (/\.(mp4|ogg)$/.test(path)) {
                return "video";
            }
        ...
        FileUtils.getFileTypeCss = function (fileName) {
            ...
            if (/\.(jpg|png|gif|bmp)$/.test(fileName)) {
                return "image";
            }
