namespace Tools.Common
{
    ...
    public static class ApplicationStateExtension
    {
        ...
        public static void RemoveApplicationState(this HttpApplicationState appState, string objectName, int syncCheckMinutes = 0)
        {
            appState.Lock();
            if (appState[objectName] != null)
            {
                appState.Remove(objectName);
            }
            if (appState[objectName + "LastSync"] == null || DateTime.Now.Subtract(((DateTime)appState[objectName + "LastSync"])).TotalMinutes >= syncCheckMinutes)
            {
                appState.Remove(objectName + "LastSync");
            }
            appState.UnLock();
        }
